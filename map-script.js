// map-script.js — island map, no shimmer, camera bounded to grid.
...
function init3DMap() {
    ...
    // Grid layout (defined early so pan bounds can reference them)
    const GRID = 40, TILE_W = 10, GAP = 0.15;
    const TOTAL  = GRID * TILE_W;           // 400 world units
    const OFFSET = TOTAL / 2 - TILE_W / 2;
    const HALF   = TOTAL / 2;               // pan clamped to ±HALF
    ...
    for (let row = 0; row < GRID; row++) {
        for (let col = 0; col < GRID; col++) {
            if (!landMask[row][col]) continue; // water tiles = just the ocean plane below
            ...
            // Cliff fill below shore so no gap above the ocean surface
            if (shore) {
    ...
    // ── ONE unified ocean plane — no per-tile water meshes, no z-fighting ─────
    // Sits at y = 0. The land tiles start at y ≈ elev – 0.5, so shore tops are
    // always above water; cliffs extend downward to seal any gap.
    ...
    oceanMesh.position.y = 0;    // single authoritative water level — nothing else at this y
    scene.add(oceanMesh);

    // ── Subtle grid lines on land only (no square outer border) ───────────────
    ...
    function updateDayNight(dt) {
        ...
        // Ocean colour shifts with time of day — single mat, no shimmer
    ...
    // ── Gentle ocean swell (single plane, so no shimmer between layers) ────────
    let waveT = 0;
    function updateWaves(dt) {
        waveT += dt;
        // Subtle bob — small amplitude so it never clips through shore cliffs
